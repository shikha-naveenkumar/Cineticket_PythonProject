...
def marvel1():
    ...
    mycur.execute("CREATE TABLE IF NOT EXISTS ticket1 (act_usr Varchar(100),name Varchar(100), ticket_id Varchar(20) PRIMARY KEY, ticket_date Varchar(100), Movie_name Varchar(100), Amount Varchar(30),trx_id Varchar(20))")
    
    #cursor.execute("DROP TABLE MOVIE_AMT ")
    mycur.execute("CREATE TABLE IF NOT EXISTS MOVIE_AMT (act_mov Varchar(100),act_amt Varchar(100),tx_id Varchar(20))")
    ...
    ##########################
    image1 = Image.open("The_marvel.png")
    test = ImageTk.PhotoImage(image1)
    label1s = tk.Label(parent,image=test)
    # Position image
    label1s.place(x=20, y=110)
    ###########################################################################The Marvels is easy to enjoy in the moment despite its cluttered story and jumbled tonal shifts."
    #to make movie content
    win_label= Label(parent, text= "DESCRIPTION : Funny, refreshingly brief, and elevated by the chemistry of its three leads.", font= ('Roman', 17), fg= "black",bg="antiquewhite3")
    win_label.place(x=25, y=470)
    win_label= Label(parent, text= "The Marvels is easy to enjoy in the moment despite its cluttered story and jumbled tonal shifts.", font= ('Roman', 17), fg= "black",bg="antiquewhite3")
    win_label.place(x=25, y=500)
    win_label= Label(parent, text= "Cast: Brie Larson, iman vellani, teyonah parris", font= ('Roman', 17), fg= "black",bg="antiquewhite3")
    win_label.place(x=25, y=530)
    win_label= Label(parent, text= "Director: nia dacosta", font= ('Roman', 17), fg= "black",bg="antiquewhite3")
    win_label.place(x=25, y=560)
    win_label= Label(parent, text= "Producer: Kevin Feige", font= ('Roman', 17), fg= "black",bg="antiquewhite3")
    win_label.place(x=25, y=590)
    win_label= Label(parent, text= "Co-Producer: David J. Grant", font= ('Roman', 17), fg= "black",bg="antiquewhite3")
    win_label.place(x=25, y=620)
    win_label= Label(parent, text= "Screenplay: Sean Bobbitt", font= ('Roman', 17), fg= "black",bg="antiquewhite3")
    win_label.place(x=25, y=650)
    win_label= Label(parent, text= "Director Of Photography:Sean Bobbitt", font= ('Roman', 17), fg= "black",bg="antiquewhite3")
    win_label.place(x=25, y=680)
    win_label= Label(parent, text= "Music: Laura Karpman", font= ('Roman', 17), fg= "black",bg="antiquewhite3")
...
##    movies = ['Tiger 3', 'Taylor swift eras tour','Japan', 'Jigarthanda','Marvel',
##        'Wish','Wonka','Napolean','Animal','Sam Bahadur','Dunki','12th Fail']
    movies = ['Marvel']
    ...
    def gticket():
        try:
            ...
            st=str(con)
            fs = open("marvel.txt",'w')
            fs.write(str(new_seat_availability))
            fs.close()
        except Exception:
            messagebox.showwarning('Warning','Please verify the details first')
            clear()
        #### Added By Medha
        btn3['state']=NORMAL
        btn2['state']=DISABLED
    ...
    cname_txt=Entry(F1,width=15,textvariable=c_name,state='normal',font='arial 15 bold',relief=SUNKEN,bd=7)
    ...
    n_txt = Entry(F2, width=20,textvariable=person,state='normal', font='arial 15 bold', relief=SUNKEN, bd=7)
    n_txt.grid(row=2, column=1, padx=10,pady=20)
##    n_txt = Entry(F2, width=20,font='arial 15 bold', relief=SUNKEN, bd=7)
##    username_varify.place(x=10,y=20)
##    n_txt.insert(0,'No of Tickets')
##    n_txt.bind('<FocusIn>',on_enter)
##    n_txt.bind('<FocusOut>',on_leave)
