...
db = mysql.connector.connect(host="localhost",user="root",passwd="Chikoo~~1511",database="techienaman")
mycur = db.cursor()
...
def delete():
    ...
    Label(root2,image=img8,border=0,bg='white').place(x=2,y=5)
    ...
    username2 = StringVar()
    password2 = StringVar()
    l3=Label(root2, text="Existing Username:")
    l3.place(x=692,y=120)
    l4=Entry(root2,textvariable=username2)
    l4.place(x=690,y=150)
    l5=Label(root2, text="Existing Password :")
    l5.place(x=692,y=190)
    l6=Entry(root2, textvariable=password2,show="*")
    l6.place(x=690,y=210)
    btn=Button(root2,text="Check",bg="lightblue",command=verify2)
...
def deletion():
    ...
    if username_info == "":
        ...
        mycur.execute(sql, t)
        sql2="delete from bookings2 where username=(%s)"
        t2 = ([username_info])
